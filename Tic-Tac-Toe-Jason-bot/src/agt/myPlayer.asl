...
+round(Z) : next <- .findall(available(X,Y),available(X,Y),AvailableCells);  
            ...
            if (not mark(1,1,_)) {   /* Mark centre cell if empty */
                play(1,1);

			} else {
				if (L > 6 ){    				/* If there are 7 or 8 available cells (i.e opponent has only 1 marked cell),
                ...
                play(A,B);
					...
					if (myMark(1,1) & myMark(0,1) & not mark(2,1,_)){
